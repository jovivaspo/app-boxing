...
- **UI** depends on application ports, not on infra adapters. Three allowed exceptions: client components may invoke Server Actions from `src/infraestructure/actions/`; `app/` route files (Server Components) act as the composition root that wires adapters into use cases; and a UI hook may import a sanctioned browser-only infrastructure adapter (see A1 below).
- **A1 (third exception):** A UI hook MAY import and construct a browser-only, non-serializable infrastructure adapter (one whose methods touch `window`/`document`, e.g. `createGsiLoaderAdapter()`) directly, ONLY when (a) it is exposed as an overridable parameter with a module-level default so tests and callers can inject a fake at the port boundary, AND (b) the specific case is justified in that PR's description — this is not a standing blanket carve-out for any hook matching the shape. Such an adapter can only be built client-side and cannot cross the RSC boundary as a serializable prop, so an `app/` Server-Component composition root is structurally unreachable for it.
- **A2 (Server-Action placement):** When a client component invokes a Server Action (first exception above), the call MUST live in that component's `{component}.hook.ts`, never in the presentational `{component}.tsx`. Presentational `.tsx` files stay free of infrastructure and Server-Action knowledge; the hook carries all such logic.
...
Definition of done — before committing, all of these must pass: `npm run lint`, `npx tsc --noEmit`, `npm run test`. Husky enforces part of this automatically:
...
Do not bypass with `--no-verify`; CI (`.github/workflows/ci.yml`) re-runs lint, type-check, and tests on every push and PR regardless.
